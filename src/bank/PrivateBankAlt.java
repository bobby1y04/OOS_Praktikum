...
/**
 * Die Klasse PrivateBankAlt repräsentiert eine alternative Version der Klasse PrivateBank, in der die
 * getAccountBalance()-Methode mithilfe des Java-Operators instanceof berechnet wird.
 */
public class PrivateBankAlt implements Bank{
   ...
   public  void setIncomingInterest(double val) {
    ...
    public double getAccountBalance(String account) {
        // Erstmal überprüfen, ob das Konto überhaupt existiert
        if (!accountsToTransactions.containsKey(account)) {
            return 0.0;
        }
        double balance = 0.0;
        for (Transaction t : accountsToTransactions.get(account)) {
            if (t instanceof Transfer) {
                Transfer transfer = (Transfer) t; // Expliziter Typecast
                if (transfer.getSender().equals(account)) { // Unterscheidung zwischen Sender und Empfänger
                    // Sender => ausgehende Überweisung
                    balance -= transfer.calculate();
                } else {
                    // Empfänger => eingehende Überweisung bzw. Einzahlung
                    balance += transfer.calculate();
                }
            } else {
                // Zahlungen beeinflussen den Kontostand direkt
                Payment payment = (Payment) t; // Expliziter Typecast
                // Einzahlungen erhöhen den Kontostand
                // Auszahlungen verringern den Kontostand
